...
function ProductDetail() {
    let arr = [];
    let myID = useParams();
    let ID = parseInt(myID.productID);
    ...
    // Get related product 
    const data = ID < products.length && products.find(item => item.id - 1 === parseInt(myID.productID));
    const relatedTo = ID < products.length && products.filter(
    ...
    return (
        <>
            ...
            <>
                    ...
                    <Box className="desc-review-text">
                        ...
                        <Container>
                            <ul className="product-nav">
                                ...
                                <li onClick={() => toggleTab(1)} className={toggleState === 1 ? 'nav-link active' : 'nav-link'}><span>Reviews {item.hasOwnProperty("comments") ? `(${Object.values(item.comments).length})` : `(0)`}</span></li>
                            </ul>
                            <Box sx={{ flexGrow: 1 }}>
                                        ...
                                        <Box className="tab-content">
                                            ...
                                            <Box className={toggleState === 1 ? 'active disappear-text' : 'disappear-text'}>
                                                <Box className="desc-text">
                                                    {item.hasOwnProperty("comments") ? Object.values(item.comments).map((item, idx) => {
                                                        return (
                ...
                {ID >= products.length && <Typography variant='h1' sx={{ fontSize: '40px', textAlign: 'center', paddingTop: '100px', paddingBottom: '80px' }}>Product Not Found</Typography>
                }
            </>
